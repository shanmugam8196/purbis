...
const TechnologyBanner = ({ title, description, image }) => {
    return (
        <>
            {/* hero section */}
            <section className="tech-section" style={{ backgroundImage : `url('${image}')` }}>
                <Container>
                    {/* content area */}
                    <div className="hero-section-content text-center pt-5 mt-5">
                        <h1>{title}</h1>
                        <p className="fs-5 text-secondary mt-4">{description}</p>
                        <div className="mt-4 d-flex justify-content-center">
                            <RequestDemoModal name="Get Started" />
                        </div>
                    </div>
                    {/* content area */}
                </Container>
